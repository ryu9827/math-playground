export type Language = 'zh' | 'en'

export const translations = {
	zh: {
		wrongQuestions: '错题本',
		settings: '设置',
		question: '题目',
		submit: '提交',
		next: '下一题',
		correct: '正确！',
		incorrect: '错误！',
		correctAnswer: '正确答案是',
		noWrongQuestions: '还没有错题，继续加油！',

		consecutiveCorrect: '连续答对',
		times: '次',
		soundSettings: '音效设置',
		soundEnabled: '启用音效',
		soundDisabled: '禁用音效',
		soundHint: '答对时播放庆祝音效',

		goalAchieved: '目标达成！',
		goalAchievedMessage: '今日练习目标已完成',
		language: '语言',
		chinese: '中文',
		english: 'English',
		save: '保存',
		cancel: '取消',
		randomQuiz: '随机题',
		addition: '加法',
		subtraction: '减法',
		multiplication: '乘法',
		division: '除法',

		practice: '练习',
		wordProblemModeOn: '应用题',
		wordProblemModeOff: '普通题',
		operationLimits: '运算限制',
		minNumber: '下限',
		maxNumber: '上限',
		additionMinHint: '加法中两个数字都必须大于等于此值',
		additionMaxHint: '加法结果不得大于此值',
		subtractionMinuendMax: '被减数上限',
		subtractionSubtrahendMin: '减数下限',
		subtractionMinuendMaxHint: '被减数不得大于此值',
		subtractionSubtrahendMinHint: '减数必须大于等于此值',
		multiplicationMin: '乘数下限',
		multiplicationMax: '乘数上限',
		multiplicationMinHint: '两个乘数都必须大于等于此值',
		multiplicationMaxHint: '两个乘数都不得大于此值',
		divisionDividendMax: '被除数上限',
		divisionDivisorMin: '除数下限',
		divisionDivisorMax: '除数上限',
		divisionDividendMaxHint: '被除数不得大于此值',
		divisionDivisorMinHint: '除数必须大于等于此值（最小为1）',
		divisionDivisorMaxHint: '除数不得大于此值',
		minMaxError: '下限必须小于上限',
		numberSplitSettings: '拆数字设置',
		numberSplitMaxTarget: '目标数字上限',
		numberSplitMaxTargetHint: '拆数字游戏中目标数字的最大值',
	},
	en: {
		wrongQuestions: 'Wrong Questions',
		settings: 'Settings',
		question: 'Question',
		submit: 'Submit',
		next: 'Next',
		correct: 'Correct!',
		incorrect: 'Incorrect!',
		correctAnswer: 'The correct answer is',
		noWrongQuestions: 'No wrong questions yet, keep it up!',

		consecutiveCorrect: 'Consecutive correct',
		times: 'times',
		soundSettings: 'Sound Settings',
		soundEnabled: 'Sound On',
		soundDisabled: 'Sound Off',
		soundHint: 'Play celebration sound when correct',

		goalAchieved: 'Goal Achieved!',
		goalAchievedMessage: "Today's practice goal completed",
		language: 'Language',
		chinese: '中文',
		english: 'English',
		save: 'Save',
		cancel: 'Cancel',
		randomQuiz: 'Random Quiz',
		addition: 'Addition',
		subtraction: 'Subtraction',
		multiplication: 'Multiplication',
		division: 'Division',

		practice: 'Practice',
		wordProblemModeOn: 'Word Problems',
		wordProblemModeOff: 'Regular',
		operationLimits: 'Operation Limits',
		minNumber: 'Min',
		maxNumber: 'Max',
		additionMinHint: 'Both numbers must be greater than or equal to this value',
		additionMaxHint: 'Sum must not exceed this value',
		subtractionMinuendMax: 'Minuend Max',
		subtractionSubtrahendMin: 'Subtrahend Min',
		subtractionMinuendMaxHint: 'Minuend must not exceed this value',
		subtractionSubtrahendMinHint:
			'Subtrahend must be greater than or equal to this value',
		multiplicationMin: 'Multiplier Min',
		multiplicationMax: 'Multiplier Max',
		multiplicationMinHint:
			'Both multipliers must be greater than or equal to this value',
		multiplicationMaxHint: 'Both multipliers must not exceed this value',
		divisionDividendMax: 'Dividend Max',
		divisionDivisorMin: 'Divisor Min',
		divisionDivisorMax: 'Divisor Max',
		divisionDividendMaxHint: 'Dividend must not exceed this value',
		divisionDivisorMinHint: 'Divisor must be greater than or equal to this value (min 1)',
		divisionDivisorMaxHint: 'Divisor must not exceed this value',
		minMaxError: 'Min must be less than Max',
		numberSplitSettings: 'Number Split Settings',
		numberSplitMaxTarget: 'Max Target Number',
		numberSplitMaxTargetHint: 'Maximum target number in Number Split game',
	},
}
